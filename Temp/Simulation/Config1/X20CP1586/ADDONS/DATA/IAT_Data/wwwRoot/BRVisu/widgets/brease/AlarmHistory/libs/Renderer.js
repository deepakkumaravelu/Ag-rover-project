    ...
    */
    p._colorMeBlue = function () {
        if (this.widget.settings.optimize) {
            this._setStyleRowOptimized();
        } else {
            this._setStyleRowNotOptimized();
        }

    };
    /**
     * @method _setStyleRowOptimized
     * @private
     * This method will iterate over the currently displayed data set in the data table and if a row fits the criteria ("Record-Index") selected in the styling dialog,
     * it will be given the style class that corresponds to this rule.
     */
    p._setStyleRowOptimized = function () {
        ...
        for (var j = 0; j < this.widget.el.find('tbody').children().length; j += 1) {
            for (var i = 0; i < this.widget.settings.config.style.length; i += 1) {
                var currRecordIndex = this.widget.el.find('tbody').children('tr:eq(' + j + ')').data('recordindex'),
                    thatModelcurrData = this.widget.model.currentData,
                    offset = Object.keys(thatModelcurrData).find(function (k) {
                        if (thatModelcurrData[k].RecordIndex === currRecordIndex) {
                            return k;
                        }

                    });
                if (offset !== undefined) {
                    var currSev = this.widget.model.currentData[offset].sev,
                        currState = this.widget.model.currentData[offset].stn,
                        rowEligible = this._rowEligibility(currState, currSev, this.widget.settings.config.style[i]),
                        recIndex = currRecordIndex;
                    if (rowEligible) {
                        this.widget.el.find('tbody').find('[data-recordindex=' + recIndex + ']').addClass('widgets_brease_AlarmHistoryStyle_style_style' + this.widget.settings.config.style[i].namePos);
                    }
                }
            }
    ...
    /**
     * @method _setStyleRowNotOptimized
     * @private
     * This method will iterate over the currently displayed data set in the data table and if a row fits the criteria selected in the styling dialog,
     * it will be given the style class that corresponds to this rule.
     */
    p._setStyleRowNotOptimized = function () {
        if (this.widget.model.currentData === undefined || !this.dt.exists()) { return; }
        if (this.widget.model.currentData.length === 0) { return; }
